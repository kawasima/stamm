...
![Flow-metrics dashboard built from this prompt](images/metrics-dashboard.png)

*The dashboard above was generated by following this prompt and is reading live
data from a stamm server (60 days of seeded history). Every number — the CFD,
cycle-time median/p85, weekly throughput, due-date slippage, WIP age, and the
mix — is computed in the browser from stamm's observations; stamm itself
computes none of them. The two metrics it cannot compute yet are shown as
